...
### 2026-03-20 - v0.11 一次性安装 + Bug 修复

**定位调整**：
- 品牌标语：从"你的赛博伙伴"改为"你的硅基同事 — 它只为你活着"
- 域名：从 get.corivo.dev 改为 corivo.ai
- 文案风格：更拟人、温暖，减少冷冰冰的技术感

**实现功能**：
- ✅ Cold Scan：首次安装时扫描本地环境构建初始画像
- ✅ First Run：处理 pending blocks，应用规则引擎标注
- ✅ First Push：基于扫描结果生成个性化自我介绍
- ✅ macOS launchd 守护进程集成

**修复的关键 Bug**：
1. 数据库持久化问题：添加事务包装和 WAL checkpoint
2. Annotation 覆盖问题：保留 cold-scan 的原始 annotation
3. Push 消息为空：修复扫描源识别逻辑
4. Heartbeat 配置读取：支持从 config.json 读取 dbKey/dbPath
5. isDirectRun 检测：修复 import.meta.url 误触发问题

**测试状态**：
- ✅ Phase 1: npm 安装测试通过
- ✅ Phase 2: Cold Scan 测试通过（42 条数据正确持久化）
- ✅ Phase 3: First Push 测试通过（显示用户画像）
- ⏳ Phase 4: 守护进程测试（待测）
- ⏳ Phase 5: 注入规则测试（待测）
- ⏳ Phase 6: 自动更新测试（待测）
- ⏳ Phase 7: 卸载测试（待测）
