...
| `packages/plugins/hosts/*` 主机侧集成资产 | 持续演进中 |
| `packages/plugins/runtime/*` 运行时代码插件 | 早期到实验性不等 |
...
| [`packages/plugins/hosts/claude-code`](packages/plugins/hosts/claude-code) | `@corivo/claude-code` | Claude Code 主机侧集成资产 |
| [`packages/plugins/hosts/codex`](packages/plugins/hosts/codex) | `@corivo/codex` | Codex 主机侧集成资产 |
| [`packages/plugins/hosts/cursor`](packages/plugins/hosts/cursor) | `@corivo/cursor` | Cursor 主机侧集成资产 |
| [`packages/plugins/hosts/opencode`](packages/plugins/hosts/opencode) | `hosts/opencode` | 预留的 OpenCode 主机侧边界（当前不是 CLI asset-backed） |
| [`packages/plugins/runtime/openclaw`](packages/plugins/runtime/openclaw) | `@corivo/openclaw` | OpenClaw 实时采集运行时插件 |
| [`packages/plugins/runtime/opencode`](packages/plugins/runtime/opencode) | `@corivo/opencode` | OpenCode 运行时插件与可安装资产来源 |

仓库现在把 AI 工具集成拆成两棵树：

- `packages/plugins/hosts/*`：宿主工具集成资产 / host bundles
- `packages/plugins/runtime/*`：可执行的运行时代码插件 / runtime plugins

安装入口仍统一走 `corivo inject --global --<host>`，由 CLI 负责把对应宿主的资产或运行时插件安装到用户环境。
