...
| [`packages/plugins/claude-code`](packages/plugins/claude-code) | `@corivo/claude-code` | Claude Code 插件目录，当前以主机侧集成资产为主 |
| [`packages/plugins/codex`](packages/plugins/codex) | `@corivo/codex` | Codex 插件目录，当前以主机侧集成资产为主 |
| [`packages/plugins/cursor`](packages/plugins/cursor) | `@corivo/cursor` | Cursor 插件目录，当前以主机侧集成资产为主 |
| [`packages/plugins/openclaw`](packages/plugins/openclaw) | `@corivo/openclaw` | OpenClaw 插件目录，当前以运行时代码为主 |
| [`packages/plugins/opencode`](packages/plugins/opencode) | `@corivo/opencode` | OpenCode 插件目录，同时包含运行时代码与可安装资产 |

仓库现在按插件名组织：

- `packages/plugins/<plugin>`：每个插件自己的目录
- 插件目录内部再按文件职责区分主机侧资产、运行时代码、脚本与文档

安装入口统一走 `corivo host install <host>`，由 CLI 负责把对应插件的资产或运行时代码安装到用户环境。
