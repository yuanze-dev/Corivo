...
export class ContextPusher {
     ...
     * @returns 关联记忆推送文本
     */
    async pushRelated(query, limit = 5) {
        // 先搜索得到初始 block
        ...
        relatedBlocks.sort((a, b) => {
            const aAssoc = associations.find((assoc) => assoc.from_id === a.id || assoc.to_id === a.id);
            const bAssoc = associations.find((assoc) => assoc.from_id === b.id || assoc.to_id === b.id);
        ...
        const lines = limited.map((block) => {
            const assoc = associations.find((a) => a.from_id === block.id || a.to_id === block.id);
