...
corivo save --content "$CONTENT" --annotation "$ANNOTATION" --source "claude-code-hooks" --no-password >> "$LOG_FILE" 2>&1 || true
exit 0
`;
...
2. 用 \`corivo save --content ... --annotation ... --no-password\` 保存记忆。
3. 回复 \`[corivo] 已记录：...\`
`;
