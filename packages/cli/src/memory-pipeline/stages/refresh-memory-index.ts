...
export const REFRESH_MEMORY_INDEX_STAGE_ID = 'refresh-memory-index';
...
export const createRefreshMemoryIndexStage = (): MemoryPipelineStage => ({
  id: REFRESH_MEMORY_INDEX_STAGE_ID,
  async run(context: MemoryPipelineContext): Promise<PipelineStageResult> {
    ...
    const descriptor = await context.artifactStore.writeArtifact({
      runId: context.runId,
      kind: 'memory-index',
      source: REFRESH_MEMORY_INDEX_STAGE_ID,
      body: JSON.stringify({ indexes }),
    });
    setIndexRefreshMetadata(context.state, {
      stageId: REFRESH_MEMORY_INDEX_STAGE_ID,
...
const getMemoryStore = (artifactStore: MemoryPipelineArtifactStore): MemoryRootArtifactStore => {
  if (
    ...
    throw new Error('RefreshMemoryIndexStage requires an artifact store with memory file access');
  }
