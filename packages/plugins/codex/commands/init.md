...
3. Run `corivo status --no-password` to see whether the database is already initialized.
...
3. Re-run `corivo status --no-password` to confirm readiness.
...
1. Confirm `corivo status --no-password` exits successfully.
